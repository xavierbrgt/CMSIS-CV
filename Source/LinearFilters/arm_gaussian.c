...
//but to keep precision we have done the intermediate compute in q3, so basicaly int16 but the output of the filter is gray uint8
//so we need to add an extra step of conversion for the second application of the filter
...
//ARM_CV_LINEAR_OUTPUT_UINT_8 is defined in arm_linear_filter_comon.h
//It is used in arm_linear_filter_comon.c to modulate the function generated depending on the output data type
