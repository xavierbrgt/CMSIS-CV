                                     ...
                                     int high_threshold)
{
	...
	for(int y = 1; y<((ImageIn -> width)>>4)+1; y++)
	{
		...
		vst1q(&ImageOut->pData[indice], (uint8x16_t)vect_void);
		vst1q(&ImageOut->pData[indice+16], (uint8x16_t)vect_void);
		//vst1q(&ImageOut->pData[indice+32], vect_void);
	}
	...
	for(int x =2; x< 3; x++)
	{
		...
		for(int y =1; y< w-15;y+=16)
		{
			...
			vect_1x4.val[2] = vect_1x2.val[1];
			vect_1x4.val[3] = vect_2x2.val[1];
			vst4q((&Img_tmp_grad2->pData[indice3].x), vect_1x4);
			for(int p = 0; p<2; p++)
			{
				y += p*8;
				indice += p*8;
				indice3 += p*8;
				q15x8x2_t vect_buff_di = vld2q_s16(&Img_tmp_grad2->pData[indice3].x);
